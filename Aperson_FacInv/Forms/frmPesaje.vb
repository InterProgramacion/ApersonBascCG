...
#Region "Variables"
    ...
    Private _ticketFullDB As String = ""   ' ticket completo tal como está en BD (con prefijo)
    ...
    ' IdConsignatario: Inicia en -1 para indicar "Sin Asignar". 
    ' El valor 0 ES VÁLIDO (Rosanda).
    Dim IdConsignatario As Integer = -1
    Dim IdDetSolicitudDesp As Integer = 0
    Dim IdDetalleBuque As Integer = 0
    ' Nueva variable para controlar si se exige inventario o se permite paso libre
    Private _RequiereValidacionInventario As Boolean = True


    'Private WithEvents miBascula As Bascula  'Se instancia la DLL C#
    Private _serialPort As SerialPort = Nothing
    Private ReadOnly _serialLock As New Object()
    Private _cerrando As Boolean = False
    Private _bufferRecepcion As String = String.Empty
#End Region
...
#Region "Entorno"
    ...
    '    End Select
    'End Sub
    Private Sub ProcesarDatoBascula(data As String)
        If _cerrando OrElse Me.IsDisposed Then Return

        Try
            If Me.InvokeRequired Then
                If Me.IsHandleCreated Then
                    Me.BeginInvoke(New Action(Of String)(AddressOf ProcesarDatoBascula), data)
                End If
                Return
            End If
        ...
        End Try
    End Sub
    Private Sub PuertoBascula_ErrorReceived(sender As Object, e As SerialErrorReceivedEventArgs)
        ' opcional: log
    End Sub
    Private Sub PuertoBascula_DataReceived(sender As Object, e As SerialDataReceivedEventArgs)
        If _cerrando Then Return

        Try
            Dim sp As SerialPort = CType(sender, SerialPort)
            If sp Is Nothing OrElse Not sp.IsOpen Then Return

            Dim recibido As String = sp.ReadExisting()
            If String.IsNullOrEmpty(recibido) Then Return

            SyncLock _serialLock
                _bufferRecepcion &= recibido

                Dim lineas() As String = _bufferRecepcion.Split({vbCrLf, vbLf, vbCr}, StringSplitOptions.None)

                If lineas.Length > 1 Then
                    For i As Integer = 0 To lineas.Length - 2
                        Dim linea As String = lineas(i).Trim()
                        If linea <> "" Then
                            ProcesarDatoBascula(linea)
                        End If
                    Next

                    _bufferRecepcion = lineas(lineas.Length - 1)
                End If
            End SyncLock
        ...
        End Try
    End Sub
    Private Function ReabrirPuertoBascula() As Boolean
        For i As Integer = 1 To 5
            Try
                CerrarPuertoBascula()
                IniciarPuertoBascula()
                Return True
        ...
        Return False
    End Function
    Private Sub CerrarPuertoBascula()
        SyncLock _serialLock
            CerrarPuertoBasculaInterno()
        End SyncLock
    End Sub

    Private Sub CerrarPuertoBasculaInterno()
        If _serialPort Is Nothing Then Return

        Try
            RemoveHandler _serialPort.DataReceived, AddressOf PuertoBascula_DataReceived
        Catch
        End Try

        Try
            RemoveHandler _serialPort.ErrorReceived, AddressOf PuertoBascula_ErrorReceived
        Catch
        End Try
    ...
    End Sub
    Private Sub IniciarPuertoBascula()
        Try
            SyncLock _serialLock
                CerrarPuertoBasculaInterno()

                _bufferRecepcion = String.Empty
                _cerrando = False

                _serialPort = New SerialPort()
                _serialPort.PortName = LibreriaGeneral.gPuertoCOM.Trim()
                _serialPort.BaudRate = LibreriaGeneral.gBitsPorSegundo
                ...
                AddHandler _serialPort.DataReceived, AddressOf PuertoBascula_DataReceived
                AddHandler _serialPort.ErrorReceived, AddressOf PuertoBascula_ErrorReceived

                LogPuerto("Antes de Open")
                _serialPort.Open()
                LogPuerto("Open OK")
            End SyncLock
        Catch ex As Exception
            LogPuerto("Open ERROR: " & ex.GetType().Name & " - " & ex.Message)
            Throw
        End Try

    End Sub
    Private Sub LogPuerto(mensaje As String)
        Try
            Dim ruta As String = "C:\Temp\pesaje_com.log"
            Dim linea As String =
            DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") &
            " | " & mensaje &
            " | User=" & Environment.UserName &
            " | PID=" & Process.GetCurrentProcess().Id.ToString() &
            " | Puerto=" & LibreriaGeneral.gPuertoCOM &
            Environment.NewLine
    ...
    Private Sub frmPesaje_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            ...
            Catch ex As Exception
                MessageBox.Show("No se pudo abrir el puerto " & LibreriaGeneral.gPuertoCOM &
                    vbCrLf & ex.Message,
                    "Puerto COM",
    ...
    ''' <summary>
    ''' Limpia un campo numérico: quita comas y espacios.
    ''' Si queda vacío, devuelve "0" para evitar SQL inválido (,  ,).
    ''' </summary>
    Private Function LimpiarNumero(valor As String) As String
        ...
        If Not Decimal.TryParse(limpio, System.Globalization.NumberStyles.Any,
    ...
    Private Sub SumaTicket()
        ...
        'Para mostrar el Dato
        dTable = Datos.consulta_reader("SELECT prefijo_cor, CAST(nticketboleta_cor + 1 AS VARCHAR(10)) AS 'Corre' FROM correlactu WHERE empresa_cor = " & LibreriaGeneral.idEmpresa & " AND tipo_cor = '" & tipoCorrelativo & "' AND estatus_cor = 'activo'")
    ...
    Private Sub btnSalir_Click(sender As Object, e As EventArgs) Handles btnSalir.Click
        'MessageBox.Show(LibreriaGeneral.gEmpresa)
        _cerrando = True
        CerrarPuertoBascula()
        Me.Close()
    End Sub
...
#End Region
    ...
    Private Sub Grabar()
        ...
        ' Usamos el ticket COMPLETO (con prefijo) para no conflictuar con registros viejos
        ' que tienen el mismo numero pero sin prefijo (de instancias anteriores).
        Dim ticketParaValidar As String = prefijoCor & txtTicket.Text
        Dim ticketSalidaValidar As String = If(Not String.IsNullOrEmpty(_ticketFullDB), _ticketFullDB, ticketParaValidar)

        Dim StringSqlE As String = <sqlExp>
                                          SELECT Ticketboleta_pes, COUNT(ticketboleta_pes) As datos,
											 'ENTRADA' As Tipo
                                          FROM pesajes
                                          WHERE ticketboleta_pes = '<%= ticketParaValidar %>'
											 AND EstePesaje_pes = 'ENTRADA'
                                             AND (empresa_pes IS NULL OR empresa_pes = '<%= LibreriaGeneral.gEmpresa %>')
                                             GROUP by ticketboleta_pes
										 ...
										 SELECT Ticketboleta_pes, COUNT(ticketboleta_pes) As datos,
											 'SALIDA' As Tipo
                                          FROM pesajes
                                             WHERE ticketboleta_pes = '<%= ticketSalidaValidar %>'
        ...
        If Paso1 = 0 Then
            Try
                ...
                ' -- Null-check para cmbClaseCarga (evita NullReferenceException si el combo no está seleccionado)
                Dim idClaseVal As String = "0"
                If cmbClaseCarga.SelectedItem IsNot Nothing Then
                    Dim drCla As DataRowView = TryCast(cmbClaseCarga.SelectedItem, DataRowView)
                    If drCla IsNot Nothing Then
                        idClaseVal = drCla("id_cla").ToString()
                    End If
                End If
                    ...
                    StringSql = <SqlString>
                             INSERT INTO [dbo].[pesajes]
                                    ...
                                    VALUES
                                       ...
                                       ,'<%= "4" %>'
                                       ,'<%= "4" %>'
                                       ,<%= LimpiarNumero(txtPManifestado.Text) %>
                                       ,<%= LimpiarNumero(txtPTaraVehiculo.Text) %>
                                       ,<%= LimpiarNumero(txtPTaraContenedor.Text) %>
                                       ,<%= LimpiarNumero(txtPesoBruto.Text) %>
                                       ,<%= LimpiarNumero(txtPesoAraña.Text) %>
                                       ,'<%= Bascula %>'
                                       ,'<%= fechahoraFormat %>'
                                       ,'<%= Usuario %>'
                                       ,'<%= Usuario %>'
                    ...
                    StringSql = <SqlString>
                             INSERT INTO [dbo].[pesajes]
                                    ...
                                    VALUES
                                       ...
                                       ,1
                                       ,<%= detSolicitudValor %>
                                       ,'<%= cbTraslado.Text.Trim().ToUpper() %>')
                                </SqlString>
                End If
                Dim saltoPorFifoOK As Boolean = False
                Try
                    If EstaPesando = "SALIDA" AndAlso cmbTipoOperacion.Text.Trim().ToUpper() = "DESPACHO" Then

                        ' 1) Si requiere validación de inventario (flag de Detalle_Buque)
                        If _RequiereValidacionInventario AndAlso cbTraslado.Text.Trim().ToUpper() <> "EXTERNO" Then

                            ' 2) Si AUTOMATICO: correr SP y SOLO si OK permites guardar
                            If cbTraslado.Text.Trim().ToUpper() = "AUTOMATICO" Then
                                ...
                                If IdDetalleBuque <= 0 Then
                                    MessageBox.Show("No se pudo validar el inventario porque falta el Detalle de Buque (BL).",
                                ...
                                If Not Decimal.TryParse(textoPeso, NumberStyles.Any, CultureInfo.InvariantCulture, pesoReal) Then
                                    MessageBox.Show("No se pudo convertir el Peso Neto para inventario.",
                                     "Inventario", MessageBoxButtons.OK, MessageBoxIcon.Information)
                                    Return
                                End If
                                ...
                                Using con As New SqlClient.SqlConnection(Datos.srt_conexion)
                                    Try
                                        ...
                                        Using cmd As New SqlClient.SqlCommand("SP_Traslado_FIFO", con)
                                            ...
                                            cmd.Parameters.AddWithValue("@cliente_id", Val(txtNaviera.Text))
                                            cmd.Parameters.AddWithValue("@correlativo_pes", txtTicket.Text.Trim())
                                            Using reader As SqlClient.SqlDataReader = cmd.ExecuteReader()
                                                If reader.Read() Then
                                                    Dim msg As String = reader("mensaje").ToString()

                                                    If reader.GetSchemaTable().Select("ColumnName = 'resultado'").Length > 0 Then
                                                        resultado = reader("resultado").ToString().Trim().ToUpper()
                                                    Else
                                                        resultado = "" ' si por alguna razón no viene
                                                    End If
                                            ...
                                            End Using
                                            If resultado = "" Then
                                                ' Si el SP no devuelve resultado pero devolvió mensaje, asumimos OK
                                                resultado = "OK"
                                            End If
                                            ...
                                            ElseIf resultado = "AJUSTE_AUTOMATICO" Then
                                                ' Ajuste dentro del % configurado. El SP ya
                                                ' mostró el mensaje informativo al usuario.
                                                fifoOk = True

                                            ElseIf resultado = "AJUSTE_EXCEDIDO" Then
                                                ' El ajuste superó el % máximo pero el SP lo
                                                ' permitió de todos modos y guardó el registro
                                                ' con estado EXCEDIDO en dbo.ajuste.
                                                ' El MessageBox con la advertencia ya fue
                                                ' mostrado por el reader("mensaje") anterior,
                                                ' así que solo continuamos normalmente.
                                                fifoOk = True

                                            ElseIf resultado = "NO_APLICA" Then
                                                cbTraslado.Text = "NORMAL"
                                                txtTras.Text = "N"
                                                fifoOk = False
                                    ...
                                    Catch ex As SqlClient.SqlException
                                        Dim t = TraducirErrorFifo(ex)
                                        MessageBox.Show(ex.Message)
                                        MessageBox.Show(t.Mensaje, t.Titulo, MessageBoxButtons.OK, t.Icono)

                                        ' Si es información (caso de negocio), NO deberías imprimir boleta ni continuar
                                        If t.Icono = MessageBoxIcon.Information Then
                                            Textos()
                                            SumaTicket()
                                            LeerData(grdData)
                                            LeerTemporales(dataSol)
                                        ...
                                        ' Si es error técnico, igual detenemos
                                        Textos()
                                        SumaTicket()
                                        LeerData(grdData)
                                        LeerTemporales(dataSol)
                                    ...
                                    Catch ex As Exception
                                        MessageBox.Show("Ocurrió un error inesperado al procesar el traslado automático." & vbCrLf & ex.Message,
                                ...
                                Else
                                    ...
                                    ' Si NO_APLICA ya cambiaste a NORMAL, debe seguir y validar manual.
                                End If
                            End If

                            ' 3) Modo manual: validar inventario antes de guardar
                            If cbTraslado.Text.Trim().ToUpper() <> "AUTOMATICO" Then
                                If Not ValidarInventario() Then Return ' NO guardar
                            End If

                        End If
                    End If
                    'LLamamos la rutina para grabar
                    Dim errInsert As String = ""
                    If Datos.EjecutarSQL(StringSql, errInsert) Then
                        Paso = "Si"
                        If saltoPorFifoOK Then
                            GoTo SaltarInventario
                        End If
                    Else
                        MessageBox.Show("Error al guardar el pesaje en la base de datos." & vbCrLf & vbCrLf &
                                        errInsert, "Error al Guardar", MessageBoxButtons.OK, MessageBoxIcon.Error)
                        Paso = "No"
                    End If
                Catch ex As Exception
                    MessageBox.Show("ERROR inesperado: " & ex.Message, "Comprobar tabla * Pesaje",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Paso = "No"
                End Try

            Catch ex As Exception
                MessageBox.Show("ERROR al preparar el pesaje: " & ex.Message, "Comprobar tabla * Pesaje",
                                MessageBoxButtons.OK, MessageBoxIcon.Error)
                Paso = "No"
            End Try
            ...
            If EstaPesando = "ENTRADA" And Paso = "Si" Then
                ...
                If Not EsDesdeTemporal Then
                    Datos.consulta_non_queryDeta("UPDATE correlactu SET nticketboleta_cor = (SELECT CAST(nticketboleta_cor + 1 AS VARCHAR(10)) AS 'X' FROM correlactu WHERE empresa_cor = " & LibreriaGeneral.idEmpresa & " AND tipo_cor = 'DESPACHO' AND estatus_cor = 'activo') WHERE empresa_cor = " & LibreriaGeneral.idEmpresa & " AND tipo_cor = 'DESPACHO' AND estatus_cor = 'activo'")
                End If
            ...
            If Paso = "Si" Then
                If EstaPesando = "SALIDA" Then
                    ...
                    If _RequiereValidacionInventario = False OrElse cbTraslado.Text.Trim().ToUpper() = "EXTERNO" Then
                        GoTo SaltarInventario
                    End If
                    ...
                    Try
                        ...
                        If tipoOp = "DESPACHO" Then
                            ...
                            If dtInv.Rows.Count = 0 Then
                                MessageBox.Show("No hay inventario registrado para este BL en la bodega seleccionada.",
                                    "Inventario", MessageBoxButtons.OK, MessageBoxIcon.Information)
                                Return
                            End If
                            ...
                            If pesoReal > maxDespachable Then
                                MessageBox.Show(
                                    "Inventario insuficiente para el despacho." & vbCrLf & vbCrLf &
                                    "Stock actual: " & stockActual.ToString("0.00") & " kg",
                                    "Inventario",
                                    MessageBoxButtons.OK,
...
SaltarInventario:
                ...
                If MessageBox.Show("Imprimir la Boleta..?", "", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                    Try
                        ...
                        End If
                    Catch ex As Exception
                        MessageBox.Show("El pesaje se guardó correctamente, pero ocurrió un error al imprimir:" & vbCrLf & vbCrLf &
                                        ex.Message, "Error de Impresión", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    End Try
                End If
    ...
    Public Sub LeerData(ByRef grdData As DataGridView)
        ...
        Dim StringSql As String = <sqlExp>
                                    SELECT CASE WHEN LEFT(p.ticketboleta_pes, 1) LIKE '[A-Z]'
                                                THEN SUBSTRING(p.ticketboleta_pes, 4, LEN(p.ticketboleta_pes))
                                                ELSE p.ticketboleta_pes
                                           END AS ticketboleta_pes,
		                                        p.placa_pes,
		                                        SUBSTRING(p.tipooperacion_pes,1,3)
                                        From pesajes p
                                        WHERE p.baja_pes = 0
                                        AND p.Idbas_entrada_pes = '<%= LibreriaGeneral.nBascu %>'
                                        AND LTRIM(RTRIM(p.EstePesaje_pes)) = 'ENTRADA'
                                        AND NOT EXISTS (
                                            SELECT 1 FROM pesajes p2
                                            WHERE p2.ticketboleta_pes = p.ticketboleta_pes
                                              AND LTRIM(RTRIM(p2.EstePesaje_pes)) = 'SALIDA'
                                              AND p2.baja_pes = 0
                                        )
                                        ORDER BY TRY_CONVERT(BIGINT,
                                            CASE WHEN LEFT(p.ticketboleta_pes, 1) LIKE '[A-Z]'
                                                 THEN SUBSTRING(p.ticketboleta_pes, 4, LEN(p.ticketboleta_pes))
                                                 ELSE p.ticketboleta_pes
                                            END);
                                     </sqlExp>.Value
            ...
            Try
                ...
                Dim StringSql As String = <sqlExp>
                                         SELECT 
                                            ...
                                            ISNULL(db.validar_inventario, 0) AS validar_inventario
                ...
                For Each dRow As DataRow In dTable.Rows
                    ...
                    Else
                        IdConsignatario = -1 ' -1 indica que viene NULO de la BD
                    End If

                    ' --- LEER SI REQUIERE VALIDACIÓN DE INVENTARIO ---
                    If dTable.Columns.Contains("validar_inventario") AndAlso Not IsDBNull(dRow("validar_inventario")) Then
                        Dim valInv As Integer = CInt(dRow("validar_inventario"))
                        If valInv = 1 Then
                            _RequiereValidacionInventario = True
                        Else
                            _RequiereValidacionInventario = False
                        End If
                    Else
...
#Region "PesajeDll"
    ...
    Private Sub Form1_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        ...
        CerrarPuertoBascula()
    End Sub
    Private Sub frmPesaje_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        CerrarPuertoBascula()
    End Sub
...
#End Region
   ...
   SELECT 
    ...
    Private Function ValidarInventario() As Boolean
        ...
        If String.IsNullOrWhiteSpace(txtBL.Text) OrElse
           String.IsNullOrWhiteSpace(txtDuca.Text) OrElse
           IdConsignatario < 0 Then  ' Menor a 0 (es decir -1) es inválido. 0 es válido.
            _RequiereValidacionInventario = False
        End If


        If _RequiereValidacionInventario = False Then
            Return True
        End If

        Try
            ' 1. Validar que tengamos Detalle de Buque seleccionado
            If IdDetalleBuque <= 0 Then
                MessageBox.Show("Debe seleccionar un BL/Detalle de Buque para validar el inventario.", "Validación de Inventario")
                Return False
            End If
            ...
            ' Si es AUTOMATICO o EXTERNO, no validar aquí
            If cbTraslado.Text.Trim().ToUpper() = "AUTOMATICO" OrElse
               cbTraslado.Text.Trim().ToUpper() = "EXTERNO" Then Return True
        ...
        End Try
    End Function
    ' Traduce mensajes de RAISERROR (SqlException) a mensajes amigables
    Private Function TraducirErrorFifo(ex As SqlClient.SqlException) As (Titulo As String, Mensaje As String, Icono As MessageBoxIcon)

        Dim raw As String = If(ex.Message, "").Trim()

        ' Normaliza un poco para hacer match
        Dim msg As String = raw.ToUpperInvariant()

        ' 1) Casos de negocio = INFORMACIÓN
        If msg.Contains("TRASLADO AUTOMATICO NO APLICA") OrElse msg.Contains("NO_APLICA") Then
            Return ("Traslado automático", raw, MessageBoxIcon.Information)
        End If

        If msg.Contains("NO SE ENCONTRÓ DETALLE_BUQUE") OrElse msg.Contains("NO_DETALLE_BUQUE") Then
            Return ("Validación de BL", "No se encontró el BL activo para este buque y producto. Verifica que el BL esté ACTIVO y corresponda al viaje/producto.", MessageBoxIcon.Information)
        End If

        If msg.Contains("NO SE ENCONTRÓ INVENTARIO") OrElse msg.Contains("INVENTARIO DISPONIBLE") Then
            Return ("Inventario", "No hay inventario disponible para este BL en la bodega seleccionada. Si esperabas stock, revisa que el inventario esté creado y activo.", MessageBoxIcon.Information)
        End If

        If msg.Contains("NO HAY SUFICIENTE INVENTARIO EN LA BODEGA") Then
            Return ("Inventario insuficiente", "No hay inventario suficiente en la bodega para completar el despacho. Revisa el stock total en bodega o ajusta el peso a despachar.", MessageBoxIcon.Information)
        End If

        If msg.Contains("NO HAY SUFICIENTE INVENTARIO DISPONIBLE PARA COMPLETAR LA SOLICITUD") OrElse msg.Contains("DESPUÉS DE BÚSQUEDA FIFO") Then
            Return ("Inventario insuficiente", "El sistema no encontró inventario suficiente para completar el despacho bajo FIFO en esta bodega. Revisa existencias o reduce el peso.", MessageBoxIcon.Information)
        End If

        ' Ajuste excedido: diferencia supera el % máximo configurado (caso de negocio, no técnico)
        If msg.Contains("EL FALTANTE SUPERA EL MÁXIMO DE AJUSTE PERMITIDO") OrElse
           msg.Contains("SUPERA EL MÁXIMO DE AJUSTE") Then
            Return (
                "Inventario insuficiente",
                "La diferencia de inventario supera el porcentaje máximo de ajuste permitido." & vbCrLf & vbCrLf &
                "Verifique las existencias en bodega o reduzca la cantidad a despachar." & vbCrLf & vbCrLf &
                "Detalle técnico:" & vbCrLf & raw,
                MessageBoxIcon.Information
            )
        End If

        ' 2) Si no encaja en negocio, lo consideramos técnico = ERROR
        Return ("Error FIFO", "Ocurrió un error al ejecutar el traslado FIFO:" & vbCrLf & raw, MessageBoxIcon.Error)

    End Function

    Private Sub txtTomaPeso_TextChanged(sender As Object, e As EventArgs) Handles txtTomaPeso.TextChanged
